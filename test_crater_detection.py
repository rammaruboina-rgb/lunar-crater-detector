#!/usr/bin/env python3
"""
Test runner for crater detection.
...
def run_test(image_folder: str, output_csv: str, ground_truth_csv: Optional[str] = None, verbose: bool = False) -> bool:
    """
    ...
    if not Path(output_csv).exists():
        print(f"ERROR: {output_csv} not found. Run nasalunar.py first.")
        return False
    ...
    for img_name in detections_df['inputImage'].unique():
        img_detections = detections_df[detections_df['inputImage'] == img_name]
        
        ellipses: List[CraterEllipse] = []
        for _, row in img_detections.iterrows():
            try:
                cx = float(row['ellipseCenterXpx'])
                if cx == -1:
                    continue
                ellipses.append({
                    'cx': cx,
        ...
        if ground_truth_csv and Path(ground_truth_csv).exists() and scoreDetections:
            score = scoreDetections(str(img_name), ellipses, ground_truth_csv)
            if score:
                results[img_name] = score
                if verbose:
                    print(f"{img_name}")
                    print(f"  GT: {score.get('ground_truth', 0)} craters")
                    print(f"  Detected: {score.get('detected', 0)}, Matched: {score.get('matched', 0)}")
                    print(f"  Precision: {score.get('precision', 0):.3f}, Recall: {score.get('recall', 0):.3f}, F1: {score.get('f1', 0):.3f}")
    ...
    if ground_truth_csv and Path(ground_truth_csv).exists() and scoreDetections:
        ...
        total_gt = sum(r.get('ground_truth', 0) for r in results.values())
        total_detected = sum(r.get('detected', 0) for r in results.values())
        avg_f1 = sum(r.get('f1', 0) for r in results.values()) / len(results) if results else 0
        avg_iou = sum(r.get('avg_iou_score', 0) for r in results.values()) / len(results) if results else 0
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test crater detector against ground truth.')
    parser.add_argument('image_folder', nargs='?', default='lunar_images', help='Folder with PNG images')
    parser.add_argument('output_csv', nargs='?', default='solution.csv', help='Output CSV file')
    parser.add_argument('--ground_truth', type=str, help='Ground truth CSV file')
    parser.add_argument('--verbose', action='store_true', help='Verbose output')
    
    args = parser.parse_args()
    success = run_test(args.image_folder, args.output_csv, args.ground_truth, args.verbose)
